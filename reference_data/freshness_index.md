...
> **LIVE LEAGUE (as of 2026-07-24):** **3.29 Curse of the Allflame is LIVE** (launched 2026-07-24; POE_LEAGUE
> flipped to it). 3.28 Mirage ended 2026-07-20 (chars → Standard). Both 3.28 & 3.29 are past EVERY model's
...
| **🔴 RENAME: `Craicic Chimeral` → `Craicic Croaker`** | Renamed **in 3.29.0** — the imprint / Hinekora's-Lock beast. **Every pre-3.29 guide, thread and habit says "Chimeral"**; both names refer to the same beast and the wiki redirects. Don't "correct" either usage — which is right depends on the patch being read | [poewiki](https://www.poewiki.net/wiki/Craicic_Chimeral) · cache §Allflame crafting |
| **Allflame crafting (the bench)** | At **The Sovereign, below decks (Vesper)**: simulates a currency use **1–4×**, you pick one outcome. Costs **Dead Man's Sulphur**. **`Intangibility: N%` = chance it gives only ONE outcome** — a *crafting-state* property, **not a combat stat**, so PoB correctly ignores it | cache §Allflame crafting (full mechanism + per-currency table) |
| **Vestigial uniques (Legion)** | **Enshrouding Crystals** (one per faction, one armour slot each) + the **Crystal of Permutation** in the Domain → your unique becomes a *different random unique* carrying a trace of the original | cache §Vestigial uniques |
