...
| **⭐ Allflame crafting** (3.29) | A **bench at The Sovereign (below decks, Vesper)** that simulates a currency use **1-4 times and lets you pick one outcome**. Works with most currency; **Ducats are exclusive to it** | Outcomes independent and may repeat — it multiplies your *chances*, not your control | **Dead Man's Sulphur** per use, scaling with currency value AND item level (Chart ≈ 1-2k, Voyage ≈ 10-20k) | Any rarity-bearing item. ⚠ **Cannot** use: Vaal, socket or quality currency, Tainted currency, Eldritch Ichors/Embers. ✅ **`Intangibility: N%`** = the chance the bench gives only ONE outcome; it accrues **per attempt**, by currency (Exalt 8-12%, Annulment/Fracturing 25-35%). A **crafting-state property, NOT a combat stat** — PoB correctly ignores it, and a roll of it on finished gear is inert. Removable only by **Tzamoto's Ducat** (50/50 remove-or-destroy). ⭐ Some league uniques carry the implicit **`Can be Allflame Crafted as if Rare; Cannot gain Intangibility`** (e.g. *Dread Captain's Cutlass*) — they take rare mod pools, can become fractured/influenced/veiled, and are exempt from the Intangibility tax. Full mechanism + the strand technique: [`leagues/Allflame.md`](leagues/Allflame.md) §Allflame crafting |
