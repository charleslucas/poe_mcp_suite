...
| **⭐ Allflame crafting / "strand crafting"** (3.29) | Scoured bases carry **Memory Strands (0–100)** that bias *currency slams* toward T1; an **Allflame Bench** turns one slam into several **ghostly copies you choose between** (Exalt→4, Divine→4, Tailoring→3). Together: a tier-biased, multi-outcome Exalt | Strands bias **tier**, not mod selection — the pool is unchanged | **~100–180 div** for a 4-piece set incl. bases; needs **26–29% quality** bases | Top-end armour only. ⚠ Author's own caveat: **not worth below ~26% quality or for early/mid gear** — recombinate or buy instead. ✅ **`Intangibility: N%` = chance the BENCH fails to give extra copies — a crafting-state property, NOT a combat stat, so PoB correctly ignores it.** Removable only by **Tzamoto's Ducat**, which may destroy the item. Full mechanism: [`leagues/Allflame.md`](leagues/Allflame.md) §Allflame crafting |
