...
| **⭐ Allflame crafting** (3.29) | A **bench at The Sovereign (below decks, Vesper)** that simulates a currency use **1-4 times and lets you pick one outcome**. Works with most currency; **Ducats are exclusive to it** | Outcomes independent and may repeat — it multiplies your *chances*, not your control | **Dead Man's Sulphur** per use, scaling with currency value AND item level (Chart ≈ 1-2k, Voyage ≈ 10-20k) | Any rarity-bearing item. ⚠ **Cannot** use: Vaal, socket or quality currency, Tainted currency, Eldritch Ichors/Embers. ✅ ** = the chance the bench gives only ONE outcome; it accrues per attempt (Exalt 8-12%, Annul/Fracture 25-35%). A crafting-state property, **NOT a combat stat** — PoB correctly ignores it. Removable only by **Tzamoto's Ducat** (50/50 remove-or-destroy). Full mechanism + the strand technique: [](leagues/Allflame.md) §Allflame crafting |
