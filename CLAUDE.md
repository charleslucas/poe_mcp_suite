...
- **Check `reference_data/` first for cached game knowledge.** Eldritch implicit pools, crafting mods, and other slowly-changing game data live there. Read the frontmatter (`fetched:` date and `league:`) to check staleness. If outdated or missing, fetch from poewiki.net via `mcp__poemcp__fetch_wiki_page` and update the cache. The directory is gitignored — stays local.
