...
- **For exhaustive concept sweeps, Grep the text lake** (`reference_data/text_lake/` — passives/uniques/gems flat text, generated from the PoB submodule by `scripts/generate_text_lake.py`). One `grep -i minion` finds *everything* minion-related across categories, exhaustively and offline — use it before multi-call tool sweeps, and to prove absence ("no node says X"). Check its `MANIFEST.md` patch stamp; the SessionStart hook flags staleness. Local-only output — never commit (`legal_considerations.md`).
